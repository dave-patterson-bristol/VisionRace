...
if __name__ == '__main__':
    ...
    def calculateThrottle(throttlemid,  value):
        if value <= 0:
            return ((value/100 +1) * throttlemid) + 1000
        else: 
            return ((1000 - throttlemid) * (value / 100)) + throttlemid + 1000

    #generic call back
    def controlCallBack(xboxControlId, value):
        global armed, rangefinder, rate,  throttlemid,  trimtoggle,  yawtrim,  pitchtrim,  rolltrim, leftTrigger, turnRate, turnRoll, turnDuration, processQueue
        print ("Control Id = {}, Value = {}".format(xboxControlId, value))
        #Throttle
        if xboxControlId == 1: ser.write("1,{},".format(int(calculateThrottle(throttlemid,  value))).encode())
        # Trim throttle mid
        if xboxControlId == 17:
            if value[1] == 1:
                if throttlemid < 700:
                    throttlemid += 50
                    ser.write("1,{},".format(int(calculateThrottle(throttlemid,  0))).encode())
            if value[1] == -1:
                if throttlemid >= 100:
                    throttlemid -= 50
                    ser.write("1,{},".format(int(calculateThrottle(throttlemid,  0))).encode())
            if value[0] == 1:
                yawtrim += 3
                ser.write("4,{},".format(1500 + yawtrim ).encode())
            if value[0] == -1:
                yawtrim -= 3
                ser.write("4,{},".format(1500 + yawtrim ).encode())
        #Yaw
        if xboxControlId == 0: ser.write("4,{},".format(1500 + yawtrim + int(value * rate)).encode())
        if xboxControlId == 15:
            trimtoggle = value;
        if trimtoggle == 1:
            #Roll
            if xboxControlId == 3: 
                rolltrim = int(value/2)
                ser.write("2,{},".format(1500 - rolltrim).encode())
            #Pitch
            if xboxControlId == 4:
                pitchtrim = int((value-50)/2)
                ser.write("3,{},".format(1500 - pitchtrim).encode())
        else:
            #Roll
            if xboxControlId == 3: ser.write("2,{},".format((1500 - rolltrim) - int(value * rate)).encode())
            #Pitch
            if xboxControlId == 4: ser.write("3,{},".format((1500 - pitchtrim) - int(((value-50) *2) * rate)).encode()) 
        #Toggle aux1 (Armed)
        if xboxControlId == 10:
           if value ==1:
               ...
               ser.write("5,{},".format(1100 + int(armed * 800)).encode())
        ...
        if xboxControlId == 11: 
           if value ==1:
               ...
               ser.write("6,{},".format(1100 + int(rangefinder * 800)).encode())
# low manual rate
        if xboxControlId == 6:
            if value ==1:
               winsound.Beep(639,100)
...
# high manual rate       
        ...
        if xboxControlId == 8:
            if value ==1:
                ser.write("4,{},".format(1500 + yawtrim + turnRate).encode())
                ser.write("2,{},".format((1500 - rolltrim) + turnRate).encode())
                time.sleep(turnDuration)
                ser.write("4,{},".format(1500 + yawtrim).encode())
                ser.write("2,{},".format(1500 - rolltrim).encode())
# fire turn for duration
        if xboxControlId == 2:
            leftTrigger = value
        if xboxControlId == 13:
            if value ==1:
               turnDuration = leftTrigger / 10
        if xboxControlId == 12:
            if value ==1:
               turnRate = leftTrigger
        
    #setup xbox controller, set out the deadzone and scale, also invert the Y Axis (for some reason in Pygame negative is up - wierd! 
    q = Queue(maxsize=2)
    xboxCont = XboxController(q, controlCallBack, deadzone = 30, scale = 100, invertYAxis = True)
